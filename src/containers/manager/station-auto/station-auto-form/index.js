...
import {Form, Input, Button, Row, Col, Select, Table, Popconfirm, Icon} from 'antd'
...
export default class StationAutoForm extends React.PureComponent {
                        ...
                        <div className="editable-row-operations">
                            {
                                editable ?
                                    <span>
                  <a onClick={() => this.saveMeasuring(record.key)}><Icon type="save"/></a>
                  <Popconfirm title="Sure to cancel?" onConfirm={() => this.cancelMeasuring(record.key)}>
                    <a><Icon type="close"/></a>
                  </Popconfirm>
                </span>
                                    : <span> <a onClick={() => this.editMeasuring(record.key)}><Icon type="edit"/></a>
                                    <Popconfirm title="Sure to delete?"
                                                onConfirm={() => this.removeMeasuring(record.key)}>
                    <a><Icon type="delete" style={{marginLeft: '5px', color: 'red'}}/></a>
                  </Popconfirm>
                                    </span>
                            }
                        </div>
    ...
    async componentWillMount() {
        const measuringList = await CategoryApi.getMeasurings({page: 1, itemPerPage: 100000}, {})
        var rows = []
        if (measuringList) {
            measuringList.data.forEach(function (row) {
                const {key, name, unit} = row
                rows.push({key, name: name.vi || name.en, unit})
            })
        }
        var options = measuringList.data.map(d => (
            <Select.Option key={d.key} value={d.key}>{d.name.vi}</Select.Option>
        ))
        this.setState({measuringListSource: rows, measuringOps: options})
        if (this.props.initialValues)
            this.setState({measuringList: this.props.initialValues.measuringList, stationType: this.props.initialValues.objStationType })
    ...
    renderSelectColumns(text, record, column,) {
        return (
            <AutoCompleteCell
                editable={record.editable}
                value={text}
                onChange={value => this.handleChangeMeasuring(value, record.key, column)}
    ...
    handleChangeMeasuring(value, key, column) {
        ...
        const measuring = this.state.measuringListSource.filter(item => value === item.key)[0];
    ...
    handleAddRow() {
        const newRow = {key: '' + this.state.measuringList.length, name: '', unit: '', editable: true}
    ...
    render() {
        ...
        return (
            <Form onSubmit={this.handleSubmit}>
                    ...
                    <Col span={12}>
                        <FormItem label={t("stationAutoManager.form.key.label")}>
                            {getFieldDecorator('key', {
                                rules: [
                                    {required: true, message: t('stationManager.form.key.error')}
                                ]
                ...
                <Row gutter={8}>
                    <Col span={12}>
                        <FormItem label={t('stationAutoManager.form.long.label')}>
                            ...
                            })(<Input placeholder={t('stationAutoManager.form.long.placeholder')}/>)}
                        </FormItem>
                    </Col>
                    <Col span={12}>
                        <FormItem label={t('stationAutoManager.form.lat.label')}>
                            ...
                            })(<Input placeholder={t('stationAutoManager.form.lat.placeholder')}/>)}
                        </FormItem>
                    </Col>
                </Row>
                <Row gutter={8}>
                    ...
                    <Col span={12}>
                        <SelectStationType getFieldDecorator={getFieldDecorator}
                                           ...
                                           value={this.props.initialValues ? this.props.initialValues.stationType : ''}/>
                    </Col>
                </Row>
                <Row gutter={8}>
                    <Col span={12}>
                        <FormItem label={t('stationAutoManager.form.emails.label')}>
                            {getFieldDecorator('emails')(
                                <Select mode="tags" placeholder={t('stationAutoManager.form.emails.placeholder')}/>
                            )}
                        </FormItem>
                    </Col>
                    <Col span={12}>
                        <FormItem label={t('stationAutoManager.form.phones.label')}>
                            {getFieldDecorator('phones')(
                                <Select mode="tags" placeholder={t('stationAutoManager.form.phones.placeholder')}/>
                            )}
                        </FormItem>
                    </Col>
                </Row>
                ...
                <Button style={{width: '200px', right: "0"}} type="primary" onClick={this.handleAddRow}>
                    add
                </Button>
                <Table rowKey={record => record.key} bordered dataSource={this.state.measuringList}
                       columns={this.columns} rowSelection={{onSelect: this.onSelect}}/>
                <FormItem>

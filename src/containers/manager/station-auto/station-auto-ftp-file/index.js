...
export default class StationAutoFtpFile extends React.PureComponent {
    ...
    getRows() {
        ...
        let data = this.state.dataSource.map((row, index) => [
            ...
            },
            {
                content: (
                    <div>
                        {moment(row.mtime).format('YYYY/MM/DD HH:mm')}
                    </div>
                )
            },
            {
                content: (
                    <div>
                        {Math.round(row.size / 10, 24) / 100 + ' Mb'}
                    </div>
                )
            },
        ])
        if (!this.state.isFirstDirectory && this.state.breadcrumb.length > 2) {
            const backDirectory = [{ content: '' }, {
                content: <div
                    style={{ cursor: 'pointer' }}
                    onClick={this.comeBackDirectory}
                ><Icon type="folder" style={{ backgroundColor: 'yellow', cursor: 'pointer' }} /> ...</div>
            }]
            data = [backDirectory
                , ...data]
        }
                        ...
                        res.data.split('\n').map((item, key) => {
                            return <div key={key} style={{ display: 'flex' }}>
                                {item.split('\t').map((itemChild, keyChild) => {
                                    return <div style={{ width: (keyChild !== 3) ? 70 : 150 }} key={keyChild}> {itemChild} </div>
                                })}
                                <br /></div>
                        })
    ...
    async fetchData() {
        const res = await FtpApi.getFtpFiles(this.state.pagination, { path: this.state.path })
        if (res.success)
            this.setState({
                dataSource: res.data,

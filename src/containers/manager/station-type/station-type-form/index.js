...
export default class StationTypeForm extends React.PureComponent {
    ...
    handleSubmit(e) {
        ...
        this.props.form.validateFields((err, values) => {
            ...
            data.icon = values.file && values.file.length>0 ? values.file[0].response.file : values.icon
                    ...
                    <Col span={12}>
                        <FormItem label={t('stationTypeManager.form.key.label')}>
                            {getFieldDecorator('key', {
                                rules: [
                                    {required: true, message: t('stationTypeManage.form.key.error')}
                                ]
                    ...
                    <Col span={12}>
                        <FormItem label={t('stationTypeManager.form.name.label')}>
                            {getFieldDecorator('name', {
                                rules: [
                                    {required: true, message: t('stationTypeManage.form.name.error')}
                                ]
